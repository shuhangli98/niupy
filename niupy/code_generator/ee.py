...
def generator(abs_path, ncore, nocc, nact, nvir):
    ...
    S_half_0_com_cv = filter_list(S_half_0_com_cv, ncore, nocc, nact, nvir)
    S_half_0_com_CV = filter_list(S_half_0_com_CV, ncore, nocc, nact, nvir)
    S_half_1_com_c = filter_list(S_half_1_com_c, ncore, nocc, nact, nvir)
    S_half_1_com_C = filter_list(S_half_1_com_C, ncore, nocc, nact, nvir)
    S_half_minus_1_com_v = filter_list(S_half_minus_1_com_v, ncore, nocc, nact, nvir)
    S_half_minus_1_com_V = filter_list(S_half_minus_1_com_V, ncore, nocc, nact, nvir)
    S_half_act = filter_list(S_half_act, ncore, nocc, nact, nvir)
    ...
    funct_preconditioner_exact = generate_preconditioner(
        mbeq, single_space, composite_space, diagonal_type="exact", algo="ee"
    )
    funct_preconditioner_block = generate_preconditioner(
        mbeq, single_space, composite_space, diagonal_type="block", algo="ee"
    )
    funct_preconditioner_only_H = generate_preconditioner(
        mbeq, block_list, None, algo="ee"
    )
