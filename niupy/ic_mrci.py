...
class ICMRCI:
    """
    This class is based on the Forte determinant class.
    """
    ...
    def do_casci(self, ncas=None, nelecas=None):
        ...
        self.cas_dets = make_hilbert_space(
            ncas, nelecas // 2, nelecas // 2, ncore=ncore
        )
    ...
    def gen_ops_from_list(self, ops):
        self.ops = []
        ind = {
            "i": [str(_) for _ in list(range(self.ninnr))],
            "c": [str(_) for _ in list(range(self.ninnr, self.ninnr + self.ndocc))],
            "a": [
                str(_)
                for _ in list(
                    range(self.ninnr + self.ndocc, self.ninnr + self.ndocc + self.nactv)
                )
            ],
            "v": [
                str(_)
                for _ in list(range(self.ninnr + self.ndocc + self.nactv, self.nmos))
            ],
        }
    ...
    def make_eigval_problem_ic(self):
        ...
        for i in range(nbasis):
            for j in range(i + 1):
                H[i, j] = forte.overlap(
                    self.ic_basis[i], forte.apply_op(self.ham, self.ic_basis[j])
                )
                H[j, i] = H[i, j]
...
if __name__ == "__main__":
    ...
    nmo = {
        "c": slice(0, ncore),
        "a": slice(ncore, ncore + nactv),
        "v": slice(ncore + nactv, nao),
    }
    ...
    ops = w.gen_op("bra", (0, 1), "avAV", "caCA", only_terms=True) + w.gen_op(
