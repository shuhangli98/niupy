...
def de_normal_order(scalar, oei, tei, gamma1, lambda2, nmo):
    ...
    scalar2 = 0.0
    scalar2 += 0.5 * np.einsum(
        "ij,jlik,kl->", g1a_h, h2aa[hole, hole, hole, hole], g1a_h
    )
    scalar2 += 0.5 * np.einsum(
        "IJ,JLIK,KL->", g1b_h, h2bb[hole, hole, hole, hole], g1b_h
    )
    scalar2 += np.einsum("ij,jLiK,KL->", g1a_h, h2ab[hole, hole, hole, hole], g1b_h)

    scalar2 -= 0.25 * np.einsum(
        "xyuv,uvxy->", h2aa[actv, actv, actv, actv], lambda2["aaaa"]
    )
    scalar2 -= 0.25 * np.einsum(
        "XYUV,UVXY->", h2bb[actv, actv, actv, actv], lambda2["AAAA"]
    )
...
def make_hamiltonian_matrix(dets, ham):
    ...
    for i, I in enumerate(dets):
        refI = forte.SparseState({I: 1.0})
        HrefI = forte.apply_op(ham, refI)
        for j, J in enumerate(dets):
            refJ = forte.SparseState({J: 1.0})
