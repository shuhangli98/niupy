...
def make_function(key, mbeq, tensor_label):
    ...
    fname = f"{fbra}_{fket}"
    func = f"def make_{tensor_label}{fname}(Hbar, Hbar2, delta, gamma1, eta1, lambda2, lambda3, lambda4, sizes):\n"
...
def make_driver(Hmbeq, Smbeq):
    ...
    func = "def driver(Hbar, Hbar2, delta, gamma1, eta1, lambda2, lambda3, lambda4, nops, slices, sizes):\n"
    ...
    for key in Hmbeq.keys():
        ...
        func += f"\theff[slices['{bra}'],slices['{ket}']] = {factor}make_H{fname}(Hbar, Hbar2, delta, gamma1, eta1, lambda2, lambda3, lambda4, sizes)\n"
        if bra != ket:
    ...
    for key in Smbeq.keys():
        ...
        bra = key.split("|")[0].replace(" ", "").replace("+", "")
        ket = key.split("|")[1].replace(" ", "").replace("+", "")
        func += f"\tovlp[slices['{bra}'],slices['{ket}']] = {factor}make_S{fname}(Hbar, Hbar2, delta, gamma1, eta1, lambda2, lambda3, lambda4, sizes)\n"
        if bra != ket:
